...
def show_graph_with_labels(adjacency_matrix, mylabels):
    ...
    estrong = [(u,v,d) for (u,v,d) in edges if d > 0.01]
...
def show_graph_with_labels_spectual_clustering(adjacency_matrix, mylabels):
    ...
    gr.add_weighted_edges_from(estrong)
    adjacency_matrix_edge_weight[adjacency_matrix_edge_weight<0.01]=0
...
for i, (input, target) in enumerate(val_loader):
   ...
   #print(saliency)
    relu_saliency=F.relu(saliency).numpy()
    ...
    max_saliency_list=[]
    index=[]
    max_index=largest_indices(_A_*relu_saliency,10)
    ...
    for p in range(len(index)):
      for j in range(len(index[0])): 
        index[p][j]=get_key(label_dic,index[p][j])
    ...
    #show_graph_with_labels(_A_, make_label_dict(label_list))
    #show_graph_with_labels(abs_saliency, make_label_dict(label_list))
    #show_graph_with_labels(_A_*abs_saliency, make_label_dict(label_list))
    show_graph_with_labels_spectual_clustering(_A_*abs_saliency, make_label_dict(label_list))
    #show_graph_with_labels(_A_*relu_saliency, make_label_dict(label_list))
    #show_graph_with_labels_spectual_clustering(_A_*relu_saliency, make_label_dict(label_list))
    
#without threshold
    
    column=(_A_*relu_saliency).sum(axis=0)
    
    top_column_idx=column.argsort()[::-1][0:5].tolist()
    print('top5 column index.....',top_column_idx)
    for j,a in enumerate(top_column_idx):
        top_column_idx[j]=get_key(label_dic,a)
    print('top5 column.....',top_column_idx)
    
    row=(_A_*relu_saliency).sum(axis=1)
...
#with threshold  
    ...
    plt.subplot(1, 2, 2)
    plt.title("_A_*relu_saliency map{}".format(i+1))
    color_map=plt.imshow(_A_*relu_saliency, cmap=plt.cm.hot)
    plt.axis('off')
